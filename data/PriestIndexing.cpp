...
bool PriestIndexing::hasDuplicities(const super& data)
{
	const tPriestVector dataCopy = data.data();

	return (std::adjacent_find(dataCopy.begin(), dataCopy.end(),
		PriestValueEquality()) != dataCopy.end());
}

tPriestVector PriestIndexing::findDuplicities(super& data, bool leaveOneSurvivor)
{
	tPriestVector retVal;
	PriestValueEquality equality;
	const tPriestVector dataCopy = data.data();

	// find first duplicity
	tPriestVector::const_iterator duplicityStartIt = 
	...
	if (duplicityStartIt != dataCopy.end())
	{
		do 
		{
			...
			tPriestVector::const_iterator it = duplicityStartIt;
			bool allFromRangeToBeDeleted = true;

			for (; it != dataCopy.end(); ++it)
			{
				const PriestFilter priestFilter((*it)->getId());
	
				if (equality(*duplicityStartIt, *it))
				{
					if (HolyServiceIndexing::filter(
						HolyServiceIndexing::allSet().data(), &priestFilter).isEmpty())
					{
						retVal.append(*it);
					}
					else
